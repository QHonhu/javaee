...
import packclass.Product;
import packclass.ProductList;
import service.impl.ModelMap;
import service.impl.RequestMapping;
...
public class Login<Dao, ApplicationContext> {
	private HttpSession session;
	private Persion p;

@service.impl.RequestMapping("/index")
	public String login(ModelMap map ,HttpServletRequest request) {
		return "login";
	}
@service.impl.RequestMapping("/index")
public String login(ModelMap map, HttpServletRequest request, HttpServletResponse response, Class MybatisDao)
		throws NoSuchAlgorithmException {
	service.impl.ApplicationContext context = new service.impl.ApplicationContext("application-context.xml");
	p = context.getBean("person", Persion.class);
	ApplicationContext context1 = (ApplicationContext) new ClassPathXmlApplicationContext();
	Persion dao = context.getBean("mybatis",MybatisDao);
	...
	List<packclass.ProductList> pl = ((dao.MybatisDao) dao).getBoughtList();
	List<Integer> number =null;
	number =((dao.MybatisDao) dao).getTitleContentId();
...
public String edit(ModelMap map,HttpServletRequest request) throws UnsupportedEncodingException
{
	...
	ApplicationContext context = new ClassPathXmlApplicationContext("application-context.xml");
	ApplicationContext context1 = new ClassPathXmlApplicationContext("application-context-dao.xml");
	MybatisDao dao = (MybatisDao) ((service.impl.ApplicationContext) context1).getBean("mybatisDao", MybatisDao.class);
	Product p = new Product();
...
public String editSubmit(ModelMap map,HttpServletRequest request) throws UnsupportedEncodingException
{
	request.setCharacterEncoding("utf-8");
	ApplicationContext context = new ClassPathXmlApplicationContext("application-context.xml");
	Dao is = context.getBean("isLogin", Dao.class1);//这个地方有错误
	Product p = new Product();
